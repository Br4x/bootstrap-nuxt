...
interface TableItemsProcessingProps {
  items?: TableItem[]
  fields?: TableField[]
  perPage?: number
  currentPage?: number
...
function sortItems(fields?: TableField[],
...
function filterItems(items: TableItem<Record<string, any>>[],
  filter: string,
  filterable?: string[]) {
  return items.filter(
    item =>
...
function mapItems(items: Ref<TableItem[]>,
  props: TableItemsProcessingProps,
  flags: Record<string, Ref<boolean>>,
  sortBy?: Ref<string | undefined>): TableItem[] {
