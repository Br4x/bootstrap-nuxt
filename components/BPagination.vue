...
import type { PaginationPage } from '~/types/PaginationPage'
...
function sanitizeTotalRows(value: number): number {
  return Math.max(toInteger(value) || DEFAULT_TOTAL_ROWS, 0)
}
function sanitizeCurrentPage(value: number, numberOfPages: number) {
...
export default defineComponent({
  ...
  props: {
    ...
    firstNumber: { type: [Boolean, String] as PropType<boolean>, default: false },
    firstText: { type: String, default: '\u00AB' },
    hideEllipsis: { type: [Boolean, String] as PropType<boolean>, default: false },
    hideGotoEndButtons: { type: [Boolean, String] as PropType<boolean>, default: false },
    ...
    lastNumber: { type: [Boolean, String] as PropType<boolean>, default: false },
    ...
    pills: { type: [Boolean, String] as PropType<boolean>, default: false },
    prevClass: { type: [Array, String], default: () => [] },
    prevText: { type: String, default: '\u2039' },
    size: { type: String as PropType<Size>, default: 'md' },
    totalRows: { type: Number, default: DEFAULT_TOTAL_ROWS },
  },
  emits: ['update:modelValue', 'pageClick'],
  setup(props, { emit, slots }) {
    ...
    const pageClick = (event: MouseEvent, pageNumber: number) => {
      ...
      const clickEvent = new BvEvent('pageClick', {
        cancelable: true,
        target,
      })
      emit('pageClick', clickEvent, pageNumber)
    ...
    return () => {
      ...
      pages.value.forEach((page, idx) => {
        const offset
          = showFirstDots.value && props.firstNumber && pageNumbers[0] !== 1 ? 1 : 0
