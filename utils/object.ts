...
export function omit<
  A extends Record<PropertyKey, unknown>, const B extends ReadonlyArray<PropertyKey>,
>(objToPluck: A,
  keysToPluck: B | (keyof A)[]): Omit<A, B[number]> {
  return Object.keys(objToPluck)
...
export function pick<
  A extends Record<PropertyKey, unknown>, const B extends ReadonlyArray<PropertyKey>,
>(objToPluck: A,
  keysToPluck: B | (keyof A)[]): Pick<A, B[number]> {
  return [...keysToPluck].reduce(
    (memo, prop) => {
 ...
 */
export function get<TDefault = unknown>(value: any,
  path: string,
