...
export function isElement(el: HTMLElement | Element): boolean {
  return !!(el && el.nodeType === Node.ELEMENT_NODE)
}

export const getBCR = (el: HTMLElement) => (isElement(el) ? el.getBoundingClientRect() : null)

export function getActiveElement(excludes = []): Element | null {
...
export function attemptFocus(el: HTMLElement, options = {}): boolean {
  try {
    el.focus(options)
  }
  catch (e) {
 ...
 * @returns
 */
export function attemptBlur(el: HTMLElement): boolean {
  try {
    el.blur()
  }
  catch (e) {
...
export function getStyle(el: HTMLElement, prop: string) {
  return prop && isElement(el) ? el.getAttribute(prop) || null : null
}
 ...
 * @returns
 */
export function select(selector: any, root: any) {
  return (isElement(root) ? root : DOCUMENT).querySelector(selector) || null
}

export function selectAll(selector: any, root: any) {
  return Array.from([(isElement(root) ? root : DOCUMENT).querySelectorAll(selector)])
}

export function getAttr(el: HTMLElement | Element, attr: string): string | null {
...
export function getById(id: string) {
  return DOCUMENT.getElementById(id.startsWith('#') ? id.slice(1) : id) || null
}
 ...
 * @param attr
 */
export function removeAttr(el: HTMLElement, attr: string): void {
  if (attr && isElement(el))
    el.removeAttribute(attr)
}

export function isTag(tag: any, name: any): boolean {
 ...
 * @param includeRoot
 */
export function closest(selector: string, root: Element, includeRoot = false) {
  if (!isElement(root))
    return null
...
export function getTransitionDelay(element: HTMLElement) {
